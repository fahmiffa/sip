        ...
        <div class="card rounded shadow-sm">
            <div class="card-header {{($item->grant == 1) ? 'bg-success' : 'bg-primary'}} text-white small p-1">
                <div class="d-flex justify-content-between px-1">
                    <p class="my-auto">
                        <span style="font-size: 0.85rem">No Dokumen : </span>{{str_replace('SPm','BAK',str_replace('600.1.15','600.1.15/PBLT',$item->doc->nomor))}}
                    </p>
            ...
            <div class="card-body">
                <div class="row mt-3">
                    ...
                    <div class="col-md-4 col-6">
                        <h6>Kondisi</h6>
                        {{ucwords($items->kondisi)}}
                    </div>
                    <div class="col-md-4 col-6 mb-3">
                        <h6>Tingkat Permanensi</h6>
                    ...
                    <div class="col-md-4 col-6">
                        ...
                        @if($ver)
                            ...
                            <a target="_blank" href="{{ route('bak.doc', ['id'=>md5($item->id)]) }}" class="btn btn-sm btn-danger"><i class="bi bi-file-pdf"></i></a>                                                    
                        @else                                                       
                             @if($item->grant == 0)
                                <a href="{{ route('sign.news', ['id'=>md5($item->bak->id)]) }}" class="btn btn-sm btn-primary"><i class="bi bi-vector-pen"></i></a>   
                                <button type="button" class="btn btn-warning text-dark btn-sm rounded-pill">On Progress</button>    
                            @else
                                <button type="button" class="btn btn-success btn-sm rounded-pill">Success</button>
                            @endif                                 
                            <a target="_blank" href="{{ route('doc.news', ['id'=>md5($item->id)]) }}" class="btn btn-sm btn-danger"><i class="bi bi-file-pdf"></i></a>                    
                        @endif
                    </div>                     
                </div>
            </div>
